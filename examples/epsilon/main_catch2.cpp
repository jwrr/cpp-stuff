...
TEST_CASE("Verify Epsilon is small", "[Epsilon]") {
  REQUIRE(EPS::epsilon<float>() < 0.0001);
  REQUIRE(EPS::epsilon<double>() < EPS::epsilon<float>());
  REQUIRE(EPS::epsilon<long double>() < EPS::epsilon<double>());
}

TEST_CASE("Verify Epsilon is gt0", "[Epsilon]") {
  SECTION( "Verify Epsilon is greater than 0" ) {
    REQUIRE(EPS::epsilon<float>() > 0.0);
    REQUIRE(EPS::epsilon<double>() > 0.0);
    REQUIRE(EPS::epsilon<long double>() > 0.0);
  }
}
